package com.adhiwie.diary.questionnaire.posttest;
...
//		new Log().e("Last date: "+sp.getInt(SRBAI_Notification_Trigger_Date_For_Today));
        ...
        if (getNotificationTriggerCountForToday() > 0)
            return;

        if (isLastNotificationTriggeredToday())
            return;
        ...
        if (participation_days == 7 || participation_days == 14 || participation_days == 21 || participation_days == 28) {
            ...
            new NotificationMgr().triggerPriorityNotification(context, pi, 9011, "Task Journal", "Your response needed");
